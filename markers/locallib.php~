...
function markers_get_status_view($cid, $aid, $sid, $behalf) {
	...
	if ($behalf != 1 && !$admin) {
		...
		$where .= ' AND role = "' . get_string('supervisor', 'local_markers') . '"'; // only one record per student we should get
	...
	foreach ($assigns as $assign) {
		...
		foreach ($assignments as $assignment) {
			...
			$module = $DB->get_record_select('modules', 'name = "assignment"', null, '*', MUST_EXIST);
			$cm = $DB->get_record('course_modules', array ('course' => $assign->courseid, 'module' => $module->id, 'instance' => $assignment->id), '*', MUST_EXIST);
//			echo 'setupid: ' . $setup->id . ' assignid: ' . $assign->id . ' type = 0<br/>';  			
			...
			if ($statusid > 1 && $assignment->assignmenttype != "offline") {
				$submission = $DB->get_record('assignment_submissions', array('assignment' => $assignment->id, 'userid' => $assign->studentid));
					if ($submission != null) {
						...
						$output = '<font color="#000000" style="font-style: italic;">' . get_string('noSubmissionYet', 'local_markers') . '</font>';
					}
				...
				if ($statusid >= 3 && $statusid <= 5) {
					...
					if ($markermap->status == 0) {
						if ($behalf != 1)
							...
							$thatcolor = '#FF0000';
							$markerstatus = "<a style=\"color: " . $thatcolor . "\" href=\"" . $url ."\">" . $msg . "</a>";						
						}
					}
...
class marker {
	...
	public $last; // SUPERVISOR if the last role was supervisor, SECONDMARKER other wise, -1 if not given anything
	
	function __construct($mid) {
		$this->markerid = $mid;
		$this->supervisor = array();
		$this->secondmarker = array();
		$this->last = -1;
	}
}
...
/*
	To be used by assignmarkers.php.  It is an alternative to less_loaded_marker
	and although it is more complicated it is slightly faster
	@param $students - an array of user objects that are students
	@param $markers - an array of marker objects.  At two markers must exist
	@param $courseid - int - the id of the course
	@return $allocations - an array of allocations. Each allocation is an array of elements
*/
function automatic_allocation($students, $markers, $courseid) {
	$i = 0;
	$allocations = array();
	...
	foreach ($students as $stud) {
		...
		while (true) {
			if ($markers[$i]->last != SUPERVISOR && !isset($markers[$i]->secondmarker[$stud->id])) {
				// this can become a supervisor
				$supervisor = $markers[$i]->markerid;
				$markers[$i]->supervisor[$stud->id] = $stud->id;
				$markers[$i]->last = SUPERVISOR;
				break;
			}
			
			if ($markers[$i]->last != SECONDMARKER && !isset($markers[$i]->supervisor[$stud->id])) {
				// this can become a secondmarker
				$secondmarker = $markers[$i]->markerid;
				$markers[$i]->secondmarker[$stud->id] = $stud->id;
				$markers[$i]->last = SECONDMARKER;
				break;
			}	
			
			$i++;
			$i = ($i >= count($markers)) ? 0 : $i;			
		}
		
		
		// by here we have either allocated a supervisor or a second marker
		$i++;
		$i = ($i >= count($markers)) ? 0 : $i;
		while (true) {
			if ($supervisor != -1) { // need to allocate a second marker too
				if ($markers[$i]->last != SECONDMARKER && !isset($markers[$i]->supervisor[$stud->id])) {
					// this can become a secondmarker
					$secondmarker = $markers[$i]->markerid;
					$markers[$i]->secondmarker[$stud->id] = $stud->id;
			...
			else { // need to allocate a supervisor
				if ($markers[$i]->last != SUPERVISOR && !isset($markers[$i]->secondmarker[$stud->id])) {
					// this can become a supervisor
					$supervisor = $markers[$i]->markerid;
					$markers[$i]->supervisor[$stud->id] = $stud->id;
		...
		$allocations[] = array('courseid' => $courseid, 'studentid' => $stud->id, 'markerid' => $supervisor, 'role' => get_string('supervisor', 'local_markers'));
		$allocations[] = array('courseid' => $courseid, 'studentid' => $stud->id, 'markerid' => $secondmarker, 'role' => get_string('secondmarker', 'local_markers'));		
...
/*
	Delete all the assigns (allocations to markers) on a given course and its related data
*/
function delete_assigns($courseid) {
	global $DB;
	$assigns = $DB->get_records('markers_assign', array('courseid' => $courseid));
	foreach ($assigns as $assign) {
	...
	$DB->delete_records('markers_assign', array('courseid' => $courseid));
}
